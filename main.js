    ...
    function init_binding_game_info(GameInfo) {
        ...
        ui.run(() => {
            ...
            popView.text_manufactures.setText(GameInfo.manufactures.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.manufactures.value,
                    size: 2
                },
                text2: {
                    text: "/" + GameInfo.manufactures.maxValue
                }
            }));
            //trading 制造站
            ...
            popView.text_trading.setText(GameInfo.trading.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.trading.value,
                    size: 2
                },
                text2: {
                    text: "/" + GameInfo.trading.maxValue
                }
            }));
            ...
            popView.text_dormitories.setText(GameInfo.dormitories.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.dormitories.value,
                    size: 2
                },
                text2: {
                    text: "/" + GameInfo.dormitories.maxValue
                }
            }));
            //干员疲劳
            ...
            popView.text_labor.setText(GameInfo.labor.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.labor.value,
                    size: 1
                },
                text2: {
                    text: "/" + GameInfo.labor.maxValue
                }
            }));
            //剿灭
            popView.text_campaign.setText(GameInfo.campaign.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.campaign.current,
                    size: 1
                },
                text2: {
                    text: "/" + GameInfo.campaign.total
                }
            }));
            //日常
            popView.text_routineDay.setText(GameInfo.routineDay.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.routineDay.current,
                    size: 3
                },
                text2: {
                    text: "/" + GameInfo.routineDay.total
                }
            }));

            popView.text_routineWeek.setText(GameInfo.routineWeek.isNull ? "暂无数据" : Htmltext({
                text1: {
                    text: GameInfo.routineWeek.current,
                    size: 3
                },
                text2: {
                    text: "/" + GameInfo.routineWeek.total
                }
            }));
        })
    }
        ...
        ui.run(() => {
            switch (index) {
                ...
                case 1:
                    ...
                    if (shift) {
                        if (!shift.suspend) ui.jjhb.checked = true;
                    ...
                    if (setting.执行 == "自定义模块") {
                        ...
                        switch (mod_data[i].id) {
                            case '自定义':
                                break;
                            case '关闭应用':
                                if (!mod_data[i].suspend) {
                                    modular_route = mod_data[i].path;
                                }
                                break;
                            case '基建换班':
                                if (!mod_data[i].suspend) {
    ...
    if (setting.公告 == "公告") {
        ...
        if (files.exists("./lib/logs/版本更新内容.txt")) {
            $ui.post(() => {
                ...
                $ui.post(() => {
                    tool.setBackgroundRoundRounded(edition_Updates.getWindow(), {
                        radius: 0,
                    })
                ...
                if (err || res['statusCode'] != 200) {
                    ...
                    $ui.post(() => {
                        ...
                        $ui.post(() => {
                            ...
                            tool.setBackgroundRoundRounded(notice_ds.getWindow(), {
                                radius: 0,
                            })
                            $ui.post(() => {
            ...
            function qq跳转频道(频道分享链接) {
                try {
                    ...
                    app.startActivity({
                        ...
                        data: "mqqapi://forward/url?src_type=web&style=default&plg_auth=1&version=1&url_prefix=" + $base64.encode(频道分享链接),
                    });
