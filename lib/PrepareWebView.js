...
module.exports = function(webview, options) {
    ...
    resolver()
    runtime.loadDex(mainScriptPath + '/lib/java/webview-bridge.dex')
    ...
    webview.getSettings().setAppCachePath(context.getExternalFilesDir(null).getAbsolutePath() + "/cache/Webpage/"); // app缓存文件路径
    webview.getSettings().setAllowFileAccess(true); // 是否允许访问文件
    //设置 缓存模式
