...
// -------------初始化-------------
// 确保目录存在
files.ensureDir(MAIN_PATH + '/lib/logs/')
files.ensureDir(MAIN_PATH + '/lib/logs/logback/')
const LOG_TYPES = {
  VERBOSE: 'VERBOSE',
  DEBUG: 'DEBUG',
  LOG: 'LOG',
...
const PATH_CONFIG = {
  'VERBOSE': MAIN_PATH + '/lib/logs/log-verboses.log',
  'LOG': MAIN_PATH + '/lib/logs/log.log',
  'INFO': MAIN_PATH + '/lib/logs/info.log',
  'WARN': MAIN_PATH + '/lib/logs/warn.log',
  'ERROR': MAIN_PATH + '/lib/logs/error.log',
  'DEVELOP': MAIN_PATH + '/lib/logs/develop.log'
}
...
function removeOutdateBacklogFiles () {
  let logbackDirPath = MAIN_PATH + '/lib/logs/logback'
...
const clearTarget = function (originLogPath) {
  ...
  if (nameRegex.test(originLogPath)) {
    ...
    if (files.exists(originLogPath)) {
      ...
      let backLogPath = MAIN_PATH + '/lib/logs/logback/' + fileName + '.' + formatDate(new Date(timestampLastHour), 'yyyyMMddHHmm') + '.log'
...
module.exports = {
  ...
  developSaving: function (content, fileName) {
    if (_config.develop_saving_mode) {
      ...
      files.append(MAIN_PATH + '/lib/logs/' + fileName + '.log', content)
    }
  },
